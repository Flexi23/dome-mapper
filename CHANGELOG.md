...
### Fixed
- **Rhombic hexecontahedron outer glue tabs in cutline overlay** — the live canvas cutline overlay now accumulates all face clip paths into a single even-odd region so the outer glue-tab cut lines remain visible in the foldable view and export overlay for the nonconvex Rhombic Hexecontahedron.
